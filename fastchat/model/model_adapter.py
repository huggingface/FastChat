...
def get_model_adapter(model_path: str, revision: str = "main") -> BaseModelAdapter:
    ...
    if model_path not in ["gpt-4", "gpt-3.5-turbo", "claude-2", "claude-instant-1"] and is_adapter_model(model_path, revision=revision):
        print(f"Adapter weights detected! Using PeftModelAdapter for {model_path=} and {revision=}")
...
def load_model(
    ...
    if device == "cpu":
        ...
        if num_gpus != 1:
            print(f"Sharding model across {num_gpus} GPUs")
...
class PeftModelAdapter:
    ...
    def load_model(self, model_path: str, from_pretrained_kwargs: dict):
        ...
        if "base_model_path" in from_pretrained_kwargs and from_pretrained_kwargs["base_model_path"] is not None:
            base_model_path = from_pretrained_kwargs["base_model_path"]
        else:
        ...
        base_adapter = get_model_adapter(base_model_path, revision=base_model_from_pretrained_kwargs["revision"])
        print(f"Loading base model for {base_model_path=} and {base_model_from_pretrained_kwargs=}")
        base_model, tokenizer = base_adapter.load_model(
...
class H4DeepSeekAdapter(BaseModelAdapter):
    ...
    def match(self, model_path: str):
        return "deepseek" in model_path.lower() and "deepseek-coder" not in model_path.lower()
...
class H4Qwen2Adapter(BaseModelAdapter):
...
class OrpoLlamaAdapter(BaseModelAdapter):
    """The model adapter for Orpo Llama models"""

    def match(self, model_path: str):
        return "llama" in model_path.lower() and "orpo-explorers" in model_path.lower()
...
class Hermes2Adapter(BaseModelAdapter):
    ...
    def match(self, model_path: str):
        return any(
            model_str in model_path.lower()
            for model_str in ["openhermes-2.5-mistral-7b", "openhermes-2-mistral-7b", "hermes-2-pro-mistral-7b"]
        )
...
register_model_adapter(OrpoLlamaAdapter)
